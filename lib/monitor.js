...
import { App, Label } from "./constants.js";
...
export class Monitor {
    constructor (opts) {
        ...
        kc.loadFromDefault();
        this.namespace = kc
            .getContextObject(kc.currentContext)
            .namespace;
    ...
    async init () {
        ...
        this.restart_agent = this._init_restart_agent();
    ...
    _init_restarter () {
        ...
        return this.device.packets.pipe(
            ...
            rx.mergeMap(t =>
                ...
                t == "DEATH"    ? rx.of(false)  :
                rx.EMPTY),
            /* We are offline until we know otherwise */
            rx.startWith(false),
             ...
             * OK if we are online). If we are offline, start attempting
             * to restart the deployment. Abandon the previous sequence. */
            rx.switchMap(o => o ? rx.NEVER : this.restart_agent),
            /* If we get this far, we have failed to restart the agent
             * successfully in a reasonable time. */
        );
    }

    /* Attempt to restart the agent. Try every 1m for 10m. After that
     * emit a notification indicating an error. This is a cold sequence:
     * every subscription starts a new attempt. */
    _init_restart_agent () {
        return rx.interval(60*1000).pipe(
            rx.tap({
                subscribe: () => this.log("Starting Agent restart"),
                finalize: () => this.log("Abandoning Agent restart"),
            }),
            rx.switchMap(() => this.patch_agent_deployment()),
            rx.ignoreElements(),
            rx.timeout({
                first:  10*60*1000,
                with:   () => rx.of(undefined),
            }),
        );
    }

    /* Restart our Edge Agent. This patches an annotation into the
     * deployment pod template, which is what kubectl rollout restart
     * does. Return a boolean indicating success or failure. */
    patch_agent_deployment () {
        ...
        this.log("Attempting deployment restart");
        return apps.patchNamespacedDeployment(
            this.deployment, this.namespace, patch,
            /* OpenAPI is just awful... */
            undefined, undefined, undefined, undefined, undefined,
            { headers: {
                "Content-Type": "application/merge-patch+json",
            } })
        .then(res => {
            this.log("Restarted deployment");
            return true;
        })
        .catch(e =>  {
            this.log("Problem restarting deployment [%s]: %o",
                e.statusCode, e.body);
            return false;
        });
    }
    ...
    run () {
        ...
        this.restarter.subscribe(() => this.log("FAILED TO RESTART AGENT"));
    }
}
