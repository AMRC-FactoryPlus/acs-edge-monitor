...
export class Monitor {
    ...
    _init_restarter () {
        ...
        return this.device.packets.pipe(
            ...
            /* Each time we get a notification, send a request to K8s.
             * The Promise from patchND is merged into the output
             * sequence, so the result will appear downstream when we
             * get it, unless another notification comes first, when we
             * will ignore the first request in favour of the new one. */
            rx.switchMap(() => {
                ...
                const patch = { 
                 ...
                 /* This request patches an annotation into the
                 * deployment pod template, which is what kubectl
    ...
    run () {
        ...
        this.restarter.subscribe(res => 
            this.log("Restarted Agent: %o", res));
    }
}
